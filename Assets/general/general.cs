...
//				SceneManager.LoadScene("level_" + (level+1));
				...
				manageEvents.UpdateGUI.publish("textMessage", "Winner Chicken Dinner!");
				break;
		...
		manageEvents.UpdateGUI.subscribe ((object obj) => {
			...
			if(pkg.key.ToString() == "textPlayerLife") if(((uint)pkg.value) <= 0){
				manageEvents.UpdateGUI.publish("textMessage", "Ran Out Of Life");
				Invoke( "gameOver", delayBeforeGameOver);
			}
			if(pkg.key.ToString() == "textAmmo") if(((uint)pkg.value) <= 0){
				Invoke( "gameOver", delayBeforeGameOver);
				manageEvents.UpdateGUI.publish("textMessage", "Bullets Ran Out");
			}
		});
	...
	void Update () {
		...
		if (totalTimeLeft <= 0) {
			totalTimeLeft = 0f;
			Invoke( "gameOver", delayBeforeGameOver);
			manageEvents.UpdateGUI.publish("textMessage", "Time Ran Out");
		}
