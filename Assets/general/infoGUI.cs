...
public class infoGUI : MonoBehaviour {
	...
	void init() {
		...
		manageEvents.UpdateGUI.subscribe((object obj)=>{
			...
			case "textPlayerLife":
				textPlayerLife.text = "LIFE: " + pkg.value;
				break;
			case "textDestroyCount":
				if(transform != null){
					textDestroyCount = transform.Find ("textDestroyCount").gameObject.GetComponent<Text>() as Text;
					textDestroyCount.text = "ENEMY: " + pkg.value;
				}
				break;
			case "textTimeLeft":
				textTimeLeft.text = "TIME: " + Mathf.RoundToInt(float.Parse(pkg.value.ToString()));
				break;
			case "textAmmo":
				textAmmo.text = "BULLETS: " + pkg.value;
				break;
			case "textSpeedShot":
